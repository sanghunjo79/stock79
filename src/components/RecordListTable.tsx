    ...
    <div className="glass-card overflow-hidden w-full">
        ...
        <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
          ...
          <div className="relative">
            <select
              ...
              className="glass-input py-1.5 pl-8 pr-8 text-xs appearance-none bg-slate-50 text-slate-800 cursor-pointer"
            >
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white text-xs font-bold flex items-center gap-1.5 shadow-sm transition-all active:scale-95 whitespace-nowrap"
          >
      ...
      {filteredRecords.length === 0 ? (
        <div className="p-12 text-center text-slate-400">
          <History className="w-12 h-12 mx-auto mb-3 text-slate-300 opacity-60" />
          ...
          <table className="w-full text-left text-sm border-collapse min-w-[750px]">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200 text-slate-500 text-xs font-bold uppercase tracking-wider">
                ...
                <th className="py-3 px-4 text-center w-36 bg-slate-50/90 sticky right-0">관리 (수정/삭제)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredRecords.map((record) => (
                ...
                >
                  {/* 일자 */}
                  <td className="py-3.5 px-4 sm:px-6 text-xs text-slate-600 whitespace-nowrap font-mono">
                  ...
                  <td className="py-3.5 px-4 font-bold text-slate-900 whitespace-nowrap">
                    <span className="text-sm group-hover:text-brand-600 transition-colors">
                      {record.stockName}
                    </span>
                  </td>
                  ...
                  <td className="py-3.5 px-4 whitespace-nowrap">
                    <span className="px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-700 text-xs border border-slate-200 font-medium">
                      {record.category}
                    </span>
                  </td>
                  ...
                  {/* 수정 및 삭제 액션 */}
                  <td className="py-3.5 px-4 text-center whitespace-nowrap bg-white/95 group-hover:bg-slate-50/90 sticky right-0">
                    {deletingId === record.id ? (
                      <div className="flex items-center justify-center gap-1.5 animate-fade-in">
                        <button
                          onClick={() => handleConfirmDelete(record.id)}
                          className="px-2 py-1 rounded-lg bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold shadow-sm transition-all active:scale-95"
                        >
                          삭제 확정
                        </button>
                        <button
                          onClick={() => setDeletingId(null)}
                          className="px-2 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium transition-colors"
                        >
                          취소
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          ...
                          title="매수 내역 수정"
                          className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-brand-50 text-slate-700 hover:text-brand-600 border border-slate-200 hover:border-brand-200 text-xs font-medium flex items-center gap-1 transition-all"
                        >
                        ...
                        <button
                          ...
                          title="매수 내역 삭제"
                          className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-rose-50 text-slate-700 hover:text-rose-600 border border-slate-200 hover:border-rose-200 text-xs font-medium flex items-center gap-1 transition-all"
                        >
      ...
      {/* 하단 알림 안내 */}
      <div className="p-3.5 bg-slate-50/80 border-t border-slate-100 text-[11px] text-slate-500 flex items-center justify-between">
        <div className="flex items-center gap-1.5">
